...
static void continents_oncomplete(unsigned int id)
{
    ...
    for (i = 0; i < offset; i++)
    {
        ...
        if (text[i] == '\n')
        {
            ...
            continents[nentries].name = start;
...
void view_settings_timezone_setup(void)
{
    ...
    pool_create_area(&view, "area0", WIDGET_IN_DEFAULT, 0, 2, 6, 1);
    pool_create_area(&view, "area1", WIDGET_IN_DEFAULT, 0, 3, 6, 1);
    pool_create_area(&view, "area2", WIDGET_IN_DEFAULT, 0, 4, 6, 1);
    pool_create_area(&view, "area3", WIDGET_IN_DEFAULT, 0, 5, 6, 1);
    pool_create_area(&view, "area4", WIDGET_IN_DEFAULT, 0, 6, 6, 1);
    pool_create_area(&view, "area5", WIDGET_IN_DEFAULT, 6, 2, 6, 1);
    pool_create_area(&view, "area6", WIDGET_IN_DEFAULT, 6, 3, 6, 1);
    pool_create_area(&view, "area7", WIDGET_IN_DEFAULT, 6, 4, 6, 1);
    pool_create_area(&view, "area8", WIDGET_IN_DEFAULT, 6, 5, 6, 1);
    pool_create_area(&view, "area9", WIDGET_IN_DEFAULT, 6, 6, 6, 1);

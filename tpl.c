...
struct tpl_root_data
{

    int flags;
    struct tpl_pidx *pidx;
    struct tpl_mmap_rec mmap;
    char *fmt;
...
static struct tpl_node *tpl_node_new(struct tpl_node *parent);
static struct tpl_node *tpl_find_i(struct tpl_node *n, int i);
static void *tpl_cpv(void *datav, const void *data, size_t sz);
static void *tpl_extend_backbone(struct tpl_node *n);
static char *tpl_fmt(struct tpl_node *r);
static void *tpl_dump_atyp(struct tpl_node *n, struct tpl_atyp *at, void *dv);
static size_t tpl_ser_osz(struct tpl_node *n);
static void tpl_free_atyp(struct tpl_node *n,struct tpl_atyp *atyp);
static int tpl_dump_to_mem(struct tpl_node *r, void *addr, size_t sz);
static int tpl_mmap_file(char *filename, struct tpl_mmap_rec *map_rec);
...
static int tpl_serlen(struct tpl_node *r, struct tpl_node *n, void *dv, size_t *serlen);
static int tpl_unpackA0(struct tpl_node *r);
static int tpl_oops(const char *fmt, ...);
static int tpl_gather_mem(char *buf, size_t len, struct tpl_gather **gs, tpl_gather_cb *cb, void *data);
static int tpl_gather_nonblocking(int fd, struct tpl_gather **gs, tpl_gather_cb *cb, void *data);
...
struct tpl_pound_data
{

    size_t inter_elt_len;
    struct tpl_node *iter_start_node;
    size_t iternum;

};

struct tpl_hook tpl_hook = {
    tpl_oops,
    malloc,
    realloc,
    free,
...
static const struct tpl_type tpl_types[] = {
    {'r', 0},
    {'i', sizeof (int32_t)},
    {'u', sizeof (uint32_t)},
    {'c', sizeof (char)},
...
static struct tpl_node *tpl_node_new(struct tpl_node *parent)
{
    ...
    if ((n = tpl_hook.malloc(sizeof (struct tpl_node))) == NULL)
        fatal_oom();
...
char *calc_field_addr(struct tpl_node *parent, int type,char *struct_addr, int ordinal)
{

    struct tpl_node *prev;
    int offset;
    int align_sz;
...
struct tpl_node *tpl_map_va(char *fmt, va_list ap)
{
    ...
    struct tpl_node *root, *parent, *n = NULL, *preceding, *iter_start_node = NULL, *struct_widest_node = NULL, *np;
    struct tpl_pidx *pidx;
    struct tpl_pound_data *pd;
    ...
    root->data = (struct tpl_root_data*)tpl_hook.malloc(sizeof (struct tpl_root_data));

    if (!root->data)
        fatal_oom();

    memset((struct tpl_root_data *)root->data, 0, sizeof (struct tpl_root_data));
    ...
    while (*c != '\0')
    {
        ...
        case 'f':
            ...
            if (n->parent->type == TPL_TYPE_ARY)
                ((struct tpl_atyp *)(n->parent->data))->sz += tpl_types[t].sz;
        ...
        case 's':
            ...
            *(char **)(n->data) = NULL;
            ...
            if (n->parent->type == TPL_TYPE_ARY)
                ((struct tpl_atyp *)(n->parent->data))->sz += sizeof (void *);
        ...
        case '#':
            ...
            if (!applies_to_struct)
            {
                ...
                if (!(t == TPL_TYPE_BYTE || t == TPL_TYPE_INT32 || t == TPL_TYPE_UINT32 || t == TPL_TYPE_DOUBLE || t == TPL_TYPE_UINT64 || t == TPL_TYPE_INT64 || t == TPL_TYPE_UINT16 || t == TPL_TYPE_INT16 || t == TPL_TYPE_STR))
                    goto fail;
            ...
            for (peek = c; *peek == '#'; peek++)
            {
                ...
                if (applies_to_struct)
                {
                    ...
                    n->num = pound_prod;
                    n->data = tpl_hook.malloc(sizeof (struct tpl_pound_data));

                    if (!n->data)
                        fatal_oom();

                    pd = (struct tpl_pound_data *)n->data;
                    ...
                    for (np = iter_start_node; np != n; np = np->next)
                    {
                        ...
                        if (n->parent->type == TPL_TYPE_ARY)
                            ((struct tpl_atyp *)(n->parent->data))->sz += tpl_types[np->type].sz * (np->num * (n->num - 1));
                ...
                else
                {
                    ...
                    if (n->parent->type == TPL_TYPE_ARY)
                        ((struct tpl_atyp*)(n->parent->data))->sz += tpl_types[t].sz * (preceding->num - 1);

                }

                root->ser_osz += (sizeof (uint32_t) * num_contig_fxlens);
                j = ((struct tpl_root_data *)root->data)->num_fxlens;
                (((struct tpl_root_data*)root->data)->num_fxlens) += num_contig_fxlens;
                num_fxlens = ((struct tpl_root_data *)root->data)->num_fxlens;
                fxlens = ((struct tpl_root_data *)root->data)->fxlens;
                fxlens = tpl_hook.realloc(fxlens, sizeof (int) * num_fxlens);

                if (!fxlens)
                    fatal_oom();

                ((struct tpl_root_data *)root->data)->fxlens = fxlens;
            ...
            case 'B':
                ...
                n = tpl_node_new(parent);
                n->type = TPL_TYPE_BIN;
                n->addr = (struct tpl_bin *)va_arg(ap,void *);
                n->data = tpl_hook.malloc(sizeof (struct tpl_bin *));

                if (!n->data)
                    fatal_oom();

                *((struct tpl_bin **)n->data) = NULL;

                if (n->parent->type == TPL_TYPE_ARY)
                    ((struct tpl_atyp *)(n->parent->data))->sz += sizeof (struct tpl_bin);
            ...
            case 'A':
                ...
                parent = n;
                expect_lparen = 1;
                pidx = (struct tpl_pidx *)tpl_hook.malloc(sizeof (struct tpl_pidx));
                ...
                DL_ADD(((struct tpl_root_data *)(root->data))->pidx, pidx);

                n->data = (struct tpl_atyp*)tpl_hook.malloc(sizeof (struct tpl_atyp));

                if (!n->data)
                    fatal_oom();

                ((struct tpl_atyp*)(n->data))->num = 0;
                ((struct tpl_atyp*)(n->data))->sz = 0;
                ((struct tpl_atyp*)(n->data))->bb = NULL;
                ((struct tpl_atyp*)(n->data))->bbtail = NULL;
                ((struct tpl_atyp*)(n->data))->cur = NULL;
    ...
    ((struct tpl_root_data*)(root->data))->fmt = tpl_hook.malloc(strlen(fmt) + 1);

    if (((struct tpl_root_data*)(root->data))->fmt == NULL)
        fatal_oom();

    memcpy(((struct tpl_root_data*)(root->data))->fmt, fmt, strlen(fmt) + 1);
...
static int tpl_unmap_file(struct tpl_mmap_rec *mr)
{
...
static void tpl_free_keep_map(struct tpl_node *r)
{

    int mmap_bits = (TPL_RDONLY | TPL_FILE);
    int ufree_bits = (TPL_MEM | TPL_UFREE);
    struct tpl_node *nxtc, *c = r->children;
    int find_next_node = 0, looking, i;
    struct tpl_root_data *d = (struct tpl_root_data *)r->data;
    struct tpl_bin **cbin;
    char **cstr;
    size_t sz;

    if ((d->flags & mmap_bits) == mmap_bits)
        tpl_unmap_file(&d->mmap);
    else if ((d->flags & ufree_bits) == ufree_bits)
        tpl_hook.free(d->mmap.text);
    ...
    if (c)
    {
        ...
        while (c->type != TPL_TYPE_ROOT)
        {
            ...
            switch (c->type)
            {
                ...
                case TPL_TYPE_BIN:
                    cbin = (struct tpl_bin **)c->data;

                    if (*cbin)
                    {

                        if ((*cbin)->addr)
                            tpl_hook.free((*cbin)->addr);
                ...
                case TPL_TYPE_ARY:
                    c->ser_osz = 0;
                    sz = ((struct tpl_atyp *)(c->data))->sz;

                    tpl_free_atyp(c, c->data);

                    c->data = (struct tpl_atyp *)tpl_hook.malloc(sizeof (struct tpl_atyp));

                    if (!c->data)
                        fatal_oom();

                    ((struct tpl_atyp*)(c->data))->num = 0;
                    ((struct tpl_atyp*)(c->data))->sz = sz;
                    ((struct tpl_atyp*)(c->data))->bb = NULL;
                    ((struct tpl_atyp*)(c->data))->bbtail = NULL;
                    ((struct tpl_atyp*)(c->data))->cur = NULL;
...
void tpl_free(struct tpl_node *r)
{
    ...
    for (pidx = d->pidx; pidx; pidx = pidx_nxt)
    {
...
static struct tpl_node *tpl_find_i(struct tpl_node *n, int i)
{
    ...
    for (pidx = ((struct tpl_root_data *)(n->data))->pidx; pidx; pidx = pidx->next)
    {
...
static void *tpl_extend_backbone(struct tpl_node *n)
{
    ...
    struct tpl_atyp *a = (struct tpl_atyp *)n->data;
    struct tpl_backbone *bb = (struct tpl_backbone *)tpl_hook.malloc(sizeof (struct tpl_backbone) + a->sz);

    if (!bb)
        fatal_oom();

#if __STDC_VERSION__ < 199901
    bb->data = (char *)((uintptr_t)bb + sizeof (struct tpl_backbone));
#endif

    memset(bb->data, 0, a->sz);
...
static int *tpl_fxlens(struct tpl_node *r, int *num_fxlens)
{

    struct tpl_root_data *d = (struct tpl_root_data *)r->data;

    *num_fxlens = d->num_fxlens;
...
static void *tpl_dump_atyp(struct tpl_node *n, struct tpl_atyp* at, void *dv)
{
    ...
    struct tpl_backbone *bb;
    struct tpl_node *c;
    void *datav;
    uint32_t slen;
    struct tpl_bin *binp;
    char *strp;
    struct tpl_atyp *atypp;
    struct tpl_pound_data *pd;
    int i;
    size_t itermax;
    ...
    for (bb = at->bb; bb; bb=bb->next)
    {
        ...
        while (c)
        {
            ...
            switch (c->type)
            {
                ...
                case TPL_TYPE_BIN:
                    memcpy(&binp,datav,sizeof (struct tpl_bin *));

                    slen = binp->sz;
                    dv = tpl_cpv(dv,&slen,sizeof (uint32_t));
                    dv = tpl_cpv(dv,binp->addr,slen);
                    datav = (void *)((uintptr_t)datav + sizeof (struct tpl_bin *));
                ...
                case TPL_TYPE_ARY:
                    memcpy(&atypp, datav, sizeof (struct tpl_atyp *));
                ...
                case TPL_TYPE_POUND:
                    pd = (struct tpl_pound_data *)c->data;
                    itermax = c->num;
    ...
    while (c)
    {
        ...
        switch (c->type)
        {
            ...
            case TPL_TYPE_BIN:
                ...
                memcpy(&binp, c->data, sizeof (struct tpl_bin *));
...
static int tpl_dump_to_mem(struct tpl_node *r, void *addr, size_t sz)
{
    ...
    while (c)
    {
        ...
        switch (c->type)
        {
            ...
            case TPL_TYPE_BIN:
                slen = (*(struct tpl_bin **)(c->data))->sz;
                dv = tpl_cpv(dv, &slen, sizeof (uint32_t));
                dv = tpl_cpv(dv, (*(struct tpl_bin **)(c->data))->addr, slen);
            ...
            case TPL_TYPE_ARY:
                dv = tpl_dump_atyp(c, (struct tpl_atyp*)c->data, dv);
...
static int tpl_sanity(struct tpl_node *r, int excess_ok)
{
    ...
    d = ((struct tpl_root_data*)(r->data))->mmap.text;
    bufsz = ((struct tpl_root_data*)(r->data))->mmap.text_sz;
    dv = d;
    ...
    if (tpl_needs_endian_swap(dv))
        ((struct tpl_root_data*)(r->data))->flags |= TPL_XENDIAN;
    ...
    if (!(intlflags & TPL_FL_NULLSTRINGS))
        ((struct tpl_root_data*)(r->data))->flags |= TPL_OLD_STRING_FMT;

    dv = (void *)((uintptr_t)dv + 1);

    memcpy(&intlsz,dv,sizeof (uint32_t));

    if (((struct tpl_root_data*)(r->data))->flags & TPL_XENDIAN)
...
static int tpl_needs_endian_swap(void *d)
{
    ...
    char *c = (char *)d;
    int cpu_is_bigendian = tpl_cpu_bigendian();
...
char *tpl_peek(int mode, ...)
{
    ...
    struct tpl_mmap_rec mr = {0,NULL,0};
    ...
    if (mode & TPL_DATAPEEK)
    {
        ...
        if (strspn(datapeek_f, tpl_datapeek_ok_chars) < datapeek_flen)
        {
            ...
            tpl_hook.free(fmt_cpy); fmt_cpy = NULL;
...
int tpl_load(struct tpl_node *r, int mode, ...)
{
    ...
    if (((struct tpl_root_data*)(r->data))->flags & (TPL_WRONLY|TPL_RDONLY))
    {
    ...
    if (mode & TPL_FILE)
    {
        ...
        if (tpl_mmap_file(filename, &((struct tpl_root_data*)(r->data))->mmap) != 0)
        {
        ...
        if ((rc = tpl_sanity(r, (mode & TPL_EXCESS_OK))) != 0)
        {
            ...
            tpl_unmap_file(&((struct tpl_root_data*)(r->data))->mmap);

            return -1;

        }

        ((struct tpl_root_data*)(r->data))->flags = (TPL_FILE | TPL_RDONLY);
    ...
    else if (mode & TPL_MEM)
    {
        ...
        ((struct tpl_root_data*)(r->data))->mmap.text = addr;
        ((struct tpl_root_data*)(r->data))->mmap.text_sz = sz;
        ...
        ((struct tpl_root_data*)(r->data))->flags = (TPL_MEM | TPL_RDONLY);

        if (mode & TPL_UFREE)
            ((struct tpl_root_data*)(r->data))->flags |= TPL_UFREE;
    ...
    if (tpl_needs_endian_swap(((struct tpl_root_data *)(r->data))->mmap.text))
        ((struct tpl_root_data*)(r->data))->flags |= TPL_XENDIAN;
...
int tpl_Alen(struct tpl_node *r, int i)
{
    ...
    struct tpl_node *n = tpl_find_i(r, i);
    struct tpl_atyp *a;
...
static void tpl_free_atyp(struct tpl_node *n, struct tpl_atyp *atyp)
{
    ...
    struct tpl_backbone *bb = atyp->bb;
    struct tpl_backbone *bbnxt;
    struct tpl_node *c;
    void *dv;
    struct tpl_bin *binp;
    struct tpl_atyp *atypp;
    ...
    while (bb)
    {
        ...
        while (c)
        {
            ...
            switch (c->type)
            {
                ...
                case TPL_TYPE_BIN:
                    memcpy(&binp,dv,sizeof (struct tpl_bin *));
                    ...
                    dv = (void *)((uintptr_t)dv + sizeof (struct tpl_bin *));
                ...
                case TPL_TYPE_ARY:
                    memcpy(&atypp, dv, sizeof (struct tpl_atyp *));
...
static int tpl_serlen(struct tpl_node *r, struct tpl_node *n, void *dv, size_t *serlen)
{
    ...
    buf_past = ((uintptr_t)d->mmap.text + d->mmap.text_sz);
            ...
            switch (c->type)
            {
                ...
                case TPL_TYPE_STR:
                    for (fidx = 0; fidx < c->num; fidx++)
                    {
                        ...
                        if (!(((struct tpl_root_data*)(r->data))->flags & TPL_OLD_STRING_FMT))
...
static int tpl_mmap_output_file(char *filename, size_t sz, void **text_out)
{
    ...
    int fd = open(filename, O_CREAT|O_TRUNC|O_RDWR, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH);
    void *text;
...
static int tpl_mmap_file(char *filename, struct tpl_mmap_rec *mr)
{

    struct stat stat_buf;

    mr->fd = open(filename, O_RDONLY);
...
int tpl_pack(struct tpl_node *r, int i)
{
    ...
    if (((struct tpl_root_data*)(r->data))->flags & TPL_RDONLY)
        tpl_free_keep_map(r);

    ((struct tpl_root_data*)(r->data))->flags |= TPL_WRONLY;
    ...
    while (child)
    {
        ...
        switch (child->type)
        {
            ...
            case TPL_TYPE_BIN:
                slen = ((struct tpl_bin *)child->addr)->sz;
                ...
                if (slen > 0)
                {
                    ...
                    memcpy(str, ((struct tpl_bin *)child->addr)->addr, slen);
                ...
                bin = tpl_hook.malloc(sizeof (struct tpl_bin));
                ...
                if (*(struct tpl_bin**)(child->data) != NULL)
                {

                    if ((*(struct tpl_bin**)(child->data))->sz != 0)
                        tpl_hook.free((*(struct tpl_bin**)(child->data))->addr);

                    tpl_hook.free(*(struct tpl_bin**)(child->data));

                }

                memcpy(child->data, &bin, sizeof (struct tpl_bin *));

                if (datav)
                {

                    datav = tpl_cpv(datav, &bin, sizeof (struct tpl_bin *));

                    *(struct tpl_bin**)(child->data) = NULL;
            ...
            case TPL_TYPE_ARY:
                if (datav)
                {
                    ...
                    sz = ((struct tpl_atyp*)(child->data))->sz;
                    datav = tpl_cpv(datav, &child->data, sizeof (void *));
                    child->data = tpl_hook.malloc(sizeof (struct tpl_atyp));

                    if (!child->data)
                        fatal_oom();

                    ((struct tpl_atyp*)(child->data))->num = 0;
                    ((struct tpl_atyp*)(child->data))->sz = sz;
                    ((struct tpl_atyp*)(child->data))->bb = NULL;
                    ((struct tpl_atyp*)(child->data))->bbtail = NULL;
            ...
            case TPL_TYPE_POUND:
                pd = (struct tpl_pound_data*)child->data;
                itermax = child->num;
...
int tpl_unpack(struct tpl_node *r, int i)
{
    ...
    if (n->type == TPL_TYPE_ROOT)
    {
        ...
        struct tpl_root_data *nd = (struct tpl_root_data *)n->data;

        dv = tpl_find_data_start(nd->mmap.text);
    ...
    while (c)
    {
        ...
        switch (c->type)
        {
            ...
            case TPL_TYPE_BIN:
                ...
                memcpy(&(((struct tpl_bin *)c->addr)->addr), &str, sizeof (void *));
                memcpy(&(((struct tpl_bin *)c->addr)->sz), &slen, sizeof (uint32_t));
            ...
            case TPL_TYPE_ARY:
                ...
                memcpy(&((struct tpl_atyp *)(c->data))->num, dv, sizeof (uint32_t));

                if (d->flags & TPL_XENDIAN)
                    tpl_byteswap(&((struct tpl_atyp*)(c->data))->num, sizeof (uint32_t));

                ((struct tpl_atyp *)(c->data))->cur = (void *)((uintptr_t)dv + sizeof (uint32_t));
...
static int tpl_unpackA0(struct tpl_node *r)
{
    ...
    struct tpl_node *n = r;
    struct tpl_node *c = n->children;
    struct tpl_root_data *d = (struct tpl_root_data *)r->data;
    void *dv = tpl_find_data_start(d->mmap.text);
    struct tpl_pound_data *pd;
    size_t A_bytes, itermax;
    uint32_t slen;
    int rc = 1, fidx, i;
...
int tpl_gather(int mode, ...)
{
    ...
    case TPL_GATHER_NONBLOCKING:
        fd = va_arg(ap, int);
        gs = (struct tpl_gather **)va_arg(ap, void *);
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (tpl == img && img != buf)
            {
                ...
                if ((*gs = tpl_hook.malloc(sizeof (struct tpl_gather))) == NULL)
                    fatal_oom();
